...
select * from client c, dealer d where c.dealer_id = d.id;

select d.name as Dealer_name, c.name as Client_name, d.location, c.priority, s.date, s.amount
from dealer d, client c, sell s
where d.id = c.dealer_id and d.id = s.dealer_id
order by d.name;

select d.name, c.name, city
from dealer d, client c
where d.id = c.dealer_id and city = location;

select s.id, amount, c.name, city
from client c, sell s
where c.id = s.client_id and s.amount between 200 and 500;

select * from dealer;

select c.name, city, d.name, commission
from client c, dealer d
where c.dealer_id = d.id;

select c.name, city, d.name, commission
from dealer d, client c
where commission > 0.13 and d.id = c.dealer_id;
